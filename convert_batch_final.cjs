...
function processDirectory(dir) {
  ...
  for (const file of files) {
    ...
    if (fullPath.includes('node_modules') || fullPath.includes('backup_php') || fullPath.includes('react-frontend') || fullPath.includes('owasp_lab') || fullPath.includes('node-backend')) continue;
    ...
    if (fs.statSync(fullPath).isDirectory()) {
      ...
      if (hasHtml) {
        ...
        // Simple HTML extraction (if it has DOCTYPE)
