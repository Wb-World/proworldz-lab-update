...
const srcDir = 'd:/download';
const outDir = 'd:/download/react-frontend/src/pages';
const appTsxPath = 'd:/download/react-frontend/src/App.tsx';

// Files to skip
const skipFiles = ['index.php', 'dashboard.php', 'login.php', 'sidebar.php', 'header.php', 'footer.php', 'common_db.php', 'logout.php', 'download.php', 'pay.php'];

const files = fs.readdirSync(srcDir).filter(f => f.endsWith('.php') && !skipFiles.includes(f));
...
files.forEach(file => {
    ...
    // Extract body or use full content if no body
    const bodyMatch = content.match(/<body[^>]*>([\s\S]*?)<\/body>/i);
    let bodyContent = bodyMatch ? bodyMatch[1] : content;

    // Strip PHP logic (very rough, just remove it)
...
console.log("Updated App.tsx with all new routes.");
